...
HRESULT CShader::NativeConstruct_Prototype(const _tchar * pShaderFilePath, const D3D11_INPUT_ELEMENT_DESC * pElements, _uint iNumElements)
{
	...
	{
		MSGBOX("D3DCompileFromFile returned E_FAIL in CShader::NativeConstruct_Prototype");
		return E_FAIL;
	}
	...
	{
		MSGBOX("D3DX11CreateEffectFromMemory returned E_FAIL in CShader::NativeConstruct_Prototype");
		return E_FAIL;
	}
	...
	for (_uint i = 0; i < TechniqueDesc.Passes; i++)
	{
		...
		{
			MSGBOX("pPassDesc->pPass is nullptr in CShader::NativeConstruct_Prototype");
			return E_FAIL;
		}
		...
		{
			MSGBOX("m_pDevice->CreateInputLayout returned E_FAIL in CShader::NativeConstruct_Prototype");
			return E_FAIL;
		}
...
HRESULT CShader::Apply(_uint iPassIndex)
{
	if (iPassIndex >= m_Passes.size())
	{
		...
		MSGBOX("iPassIndex >= m_Passes.size in CShader::Apply");
		return E_FAIL;
	}
...
HRESULT CShader::Set_InputLayout(_uint iPassIndex)
{
	if (iPassIndex >= m_Passes.size())
	{
		MSGBOX("iPassIndex >= m_Passes.size in CShader::Set_InputLayOut");
		return E_FAIL;
	}
...
HRESULT CShader::Set_RawValue(const char * pValueName, const void * pData, _uint iLength)
{
	...
	if (nullptr == pValue)
	{
		MSGBOX("pValue is nullptr in CShader::Set_RawValue");
		return E_FAIL;
	}
...
HRESULT CShader::Set_Texture(const char * pValueName, ID3D11ShaderResourceView * pShaderResourceView)
{
	...
	if (nullptr == pValue)
	{
		MSGBOX("pValue is nullptr in CShader::Set_Texture");
		return E_FAIL;
	}
	...
	//{
	//	MSGBOX("pShaderResourceVariable is nullptr in CShader::Set_Texture");
	//	return E_FAIL;
	//}
