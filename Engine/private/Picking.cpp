...
HRESULT CPicking::Transform_ToWorldSpace()
{
	...
	_vector		vMousePos = XMVectorSet(ptMouse.x / ((rcWindowSize.right - rcWindowSize.left) * 0.5f) - 1,
										ptMouse.y / -((rcWindowSize.bottom - rcWindowSize.top) * 0.5f) + 1,
										0.f, 1.f);
...
_bool CPicking::IsNaviPick(_fvector * pLocalPoints)
{
	...
	if (TriangleTests::Intersects((_fvector)m_vRayPos, (_fvector)m_vRayDir
		, (_fvector)pLocalPoints[0], (GXMVECTOR)pLocalPoints[1], (HXMVECTOR)pLocalPoints[2], fDist))
	{
		return true;
	}
	else
