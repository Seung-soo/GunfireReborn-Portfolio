...
#pragma region VERTEX_BUFFER
	...
	{
		MSGBOX("Create_VertexBuffer returned E_FAIL in CVIBuffer_Triangle::NativeConstruct");
		return E_FAIL;
	}
...
#pragma region INDEX_BUFFER
	...
	{
		MSGBOX("Create_IndexBuffer returned E_FAIL in CVIBuffer_Triangle::NativeConstruct");
		return E_FAIL;
	}
...
HRESULT CVIBuffer_Triangle::Render(CShader * pShader, _uint iPassIndex)
{
	...
	{
		MSGBOX("nullptr == m_pDeviceContext returned E_FAIL in CVIBuffer_Triangle::Render");
		return E_FAIL;
	}
	...
	{
		MSGBOX("pShader->Set_InputLayout returned E_FAIL in CVIBuffer_Triangle::Render");
		return E_FAIL;
	}

	if (FAILED(pShader->Apply(iPassIndex)))
	{
		MSGBOX("pShader->Apply returned E_FAIL in CVIBuffer_Triangle::Render");
		return E_FAIL;
	}
...
HRESULT CVIBuffer_Triangle::Update(const _float3 * pPoints)
{
	if (nullptr == m_pDeviceContext)
	{
		MSGBOX("nullptr == m_pDeviceContext in CVIBuffer_Triangle::Update");
		return E_FAIL;
	}
