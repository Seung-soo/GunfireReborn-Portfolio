#include "stdafx.h"
#include "..\public\GhostBullet.h"
#include "Player_Hand.h"
#include "GameInstance.h"
#include "Particle_Effect.h"

CGhostBullet::CGhostBullet(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	: CGameObject(pDevice, pDeviceContext)
{

}

CGhostBullet::CGhostBullet(const CGhostBullet & rhs)
	: CGameObject(rhs)
{
}

HRESULT CGhostBullet::NativeConstruct_Prototype()
{
...
HRESULT CGhostBullet::NativeConstruct(void * pArg)
{
	...
	{
		MSGBOX("SetUp_Components returned E_FAIL in CGhostBullet::NativeConstruct");
		return E_FAIL;
	}
	...
	// ÀÌÆåÆ® »ý¼º
	if (FAILED(g_pGameInstance->Add_GameObject(g_eCurrentLevel, TEXT("Layer_Effect"), TEXT("Prototype_GameObject_GhostBulletEffect"), this)))
	{
...
_int CGhostBullet::Tick(_double TimeDelta)
{
	if (true == m_bDead)
		return DEAD;
	...
	{
		static_cast<CPlayer_Hand*>(g_pGameInstance->Get_Back(LEVEL_STATIC, TEXT("Layer_Player_Hand")))->Hit(100.f);
		m_bDead = true;
	}
...
_int CGhostBullet::LateTick(_double TimeDelta)
{
...
HRESULT CGhostBullet::Render()
{
	...
	{
		MSGBOX("SetUp_ConstantTable returned E_FAIL in CGhostBullet::Render");
		return E_FAIL;
	}
...
HRESULT CGhostBullet::SetUp_Components()
{
	/* For.Com_Renderer */
	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Renderer"), TEXT("Com_Renderer"), (CComponent**)&m_pRendererCom)))
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CGhostBullet::SetUp_Components(Renderer)");
		return E_FAIL;
	}
	/* For.Com_Shader */
	...
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CGhostBullet::SetUp_Components(Shader)");
		return E_FAIL;
	}
	...
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CGhostBullet::SetUp_Components(Model)");
		return E_FAIL;
	}
	/* For.Com_Collider */
	...
	{
		MSGBOX("__super::Add_Component returned E_FAIL in CGhostBullet::SetUp_Components(Collider)");
		return E_FAIL;
	}
...
HRESULT CGhostBullet::SetUp_ConstantTable()
{
	if (nullptr == m_pShaderCom)
	{
		MSGBOX("m_pShaderCom is nullptr in CGhostBullet::SetUp_ConstantTable");
		return E_FAIL;
	}
	...
	{
		MSGBOX("m_pTransformCom->Bind_OnShader returned E_FAIL in CGhostBullet::SetUp_ConstantTable");
		return E_FAIL;
	}
	if (FAILED(m_pShaderCom->Set_RawValue("g_ViewMatrix", &pGameInstance->Get_TransformFloat4x4_TP(CPipeLine::D3DTS_VIEW), sizeof(_float4x4))))
	{
		MSGBOX("m_pShaderCom->Set_RawValue returned E_FAIL in CGhostBullet::SetUp_ConstantTable(View)");
		return E_FAIL;
	}
	if (FAILED(m_pShaderCom->Set_RawValue("g_ProjMatrix", &pGameInstance->Get_TransformFloat4x4_TP(CPipeLine::D3DTS_PROJ), sizeof(_float4x4))))
	{
		MSGBOX("m_pShaderCom->Set_RawValue returned E_FAIL in CGhostBullet::SetUp_ConstantTable(Proj)");
		return E_FAIL;
	}
...
CGhostBullet * CGhostBullet::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CGhostBullet*	pInstance = new CGhostBullet(pDevice, pDeviceContext);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSGBOX("Failed to Created CGhostBullet");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CGhostBullet::Clone(void * pArg)
{
	CGhostBullet*	pInstance = new CGhostBullet(*this);
